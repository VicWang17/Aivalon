...
**当前位置**：**D-3 房间路由已全部闭环**。一致性哈希环（DEVLOG C05）、节点注册 + 心跳租约（C06）、路由接入读写请求路径（016~018 三个"单机跑不出来且不报错"的 bug）、双节点演练（转发正确性 + kill -9 后 5.6s 接管续打）。**下一步进 D-4（AI 双引擎 + 降级开关）**。
...
- [x] 房间路由：一致性哈希（160 虚拟节点/物理节点），节点表 + 地址表存 Redis，心跳租约维护存活视图；读写两侧均接入转发，一跳封顶防环（DEVLOG C05/C06、016~018）
- [x] 节点宕机迁移：房间漂移到健康节点，状态从 Redis 快照恢复（双节点演练实测 kill -9 后 5.6s 接管并续打，状态无回退）
...
- [x] kill 演练：**进程**宕机恢复、事件不丢不重（DEVLOG 013）；房间跨**节点**迁移已过（`bench/drill_room_routing.py`，kill -9 归属节点 → 5.6s 接管 → 续打无回退，DEVLOG D-3 演练结果）
