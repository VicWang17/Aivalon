# 这个文件是Alembic迁移工具的运行环境配置，负责加载应用配置、建立数据库连接并执行迁移脚本，是数据库版本控制的核心入口。
import os
import sys
...
# 2. 将 backend 目录加入 sys.path，以便能导入 app 模块
...
from app.models.outbox import OutboxEvent # 导入 Outbox 模型以便 Alembic 识别
